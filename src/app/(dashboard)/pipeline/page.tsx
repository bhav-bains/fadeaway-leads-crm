...
import { Search, MapPin, Download, Send, Plus, ChevronLeft, ChevronRight, Mail, Phone, Globe, Star, CheckCircle2, XCircle, FileJson, Eye, Building2 } from "lucide-react";
    ...
    return (
        <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="bg-white p-3 rounded-md shadow-sm border mb-2 cursor-grab active:cursor-grabbing text-sm group">
            <div className="flex justify-between items-start mb-1">
                ...
                <Badge variant={lead.scores?.[0]?.score_overall >= 60 ? 'default' : 'secondary'} className="text-[10px] px-1 h-4 shrink-0">{lead.scores?.[0]?.score_overall || 0}/100</Badge>
            </div>
            <div className="text-xs text-muted-foreground mb-2 flex flex-col gap-1">
                <div className="flex items-center gap-1"><MapPin className="h-3 w-3" />{lead.city}</div>
                {lead.niche && <div className="flex items-center gap-1 text-primary/80 font-medium"><Building2 className="h-3 w-3" />{lead.niche}</div>}
            </div>
            ...
            <div className="mt-3" onPointerDown={e => e.stopPropagation()}>
                <Button 
                    variant="outline" 
                    className="w-full h-7 text-xs bg-slate-50 hover:bg-slate-100 text-slate-600 border-slate-200 shadow-sm"
...
function KanbanColumn({ stage, leads, onOpenLead }: { stage: string, leads: any[], onOpenLead: (l: any) => void }) {
    ...
    return (
        <div ref={setNodeRef} className="bg-muted/40 rounded-lg border w-[300px] min-w-[300px] flex flex-col h-full shrink-0">
            <div className="p-3 border-b bg-muted/60 flex justify-between items-center shrink-0">
                <h3 className="font-semibold text-sm">{stage}</h3>
                <Badge variant="secondary" className="px-1.5 min-w-[20px] justify-center">{leads.length}</Badge>
            </div>
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-8rem)] gap-4">
                                            ...
                                            return (
                                                <TableRow key={lead.id} className="cursor-pointer group hover:bg-muted/30" onClick={() => setActiveLead(lead)}>
                                                    ...
                                                    <TableCell className="text-center">
                                                        <Tooltip>
                                                            <TooltipTrigger>
                                                                <Badge variant={scoreObj.score_overall >= 60 ? 'default' : (scoreObj.score_overall >= 30 ? 'secondary' : 'outline')} className="px-2 font-bold cursor-help">
                                                            ...
                                                            <TooltipContent side="left" className="p-3 shadow-xl">
                                                                <div className="space-y-1.5 text-xs font-medium">
                                                                    ...
                                                                    <div className="border-t pt-1 mt-1 flex justify-between font-bold"><span>Total Score</span> <span>{scoreObj.score_overall || 0}/100</span></div>
                                                                </div>
                                                            </TooltipContent>
                                                        </Tooltip>
                                                    </TableCell>
            ...
            <Sheet open={!!activeLead} onOpenChange={(o) => { if (!o) setActiveLead(null) }}>
                <SheetContent side="right" className="!w-[90vw] sm:!w-[50vw] sm:!max-w-[50vw] overflow-y-auto p-0">
                    {activeLead && (
                        <div className="flex flex-col h-full bg-slate-50">
                            <div className="px-6 py-6 bg-white border-b sticky top-0 z-10 shadow-sm">
                                ...
                                <SheetDescription className="mt-1 flex flex-col sm:flex-row sm:items-center text-sm gap-2 sm:gap-4">
                                    <span className="flex items-center"><MapPin className="h-3.5 w-3.5 mr-1" /> {activeLead.address}, {activeLead.city}</span>
                                    {activeLead.niche && <span className="flex items-center text-primary/80 font-medium"><Building2 className="h-3.5 w-3.5 mr-1" /> {activeLead.niche}</span>}
                                </SheetDescription>
                            ...
                            <div className="p-6 space-y-6 flex-1">
                                ...
                                <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
                                    <div className="bg-muted/50 px-4 py-2 border-b flex justify-between items-center">
                                        ...
                                        <Badge variant={activeLead.scores?.[0]?.score_overall >= 60 ? 'default' : 'secondary'}>{activeLead.scores?.[0]?.score_overall || 0}/100</Badge>
                                    </div>
